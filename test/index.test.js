...
test('https://vs-cmaf-push-ww-live.akamaized.net/x=4/i=urn:bbc:pips:service:bbc_news_channel_hd/iptv_hd_abr_v1.mpd', async () => {
  const result = await check('https://vs-cmaf-push-ww-live.akamaized.net/x=4/i=urn:bbc:pips:service:bbc_news_channel_hd/iptv_hd_abr_v1.mpd')
  expect(result).toBe(true)
})
...
test('https://beyaztv.daioncdn.net/beyaztv/beyaztv.m3u8?app=fcd5c66b-da9d-44ba-a410-4f34805c397d&ce=3', async () => {
  const result = await check('https://beyaztv.daioncdn.net/beyaztv/beyaztv.m3u8?app=fcd5c66b-da9d-44ba-a410-4f34805c397d&ce=3')
  expect(result).toBe(true)
})
...
test('http://185.234.111.229:8000/play/a01m', async () => {
  const result = await check('http://185.234.111.229:8000/play/a01m')
  expect(result).toBe(true)
})
